    ...
    <div class="w-full max-w-md rounded-3xl border border-slate-700/50 bg-slate-900/70 p-8 shadow-2xl backdrop-blur">
      <div class="mb-6 text-center">
        ...
        <p class="mt-2 text-sm text-slate-300">
          Gunakan akun demo untuk mencoba seluruh fitur dashboard dan menu pegawai.
        </p>
        <div class="mt-4 rounded-2xl border border-slate-700 bg-slate-900/80 px-4 py-3 text-left text-xs text-slate-300">
          <p class="font-semibold text-slate-100">Akun Demo</p>
          <p><span class="text-slate-400">Email:</span> admin@gmail.com</p>
          <p><span class="text-slate-400">Kata sandi:</span> admin123</p>
        </div>
      </div>
      <form class="space-y-5" @submit.prevent="onSubmit">
        <div>
          <label for="nrp" class="block text-sm font-medium text-slate-200">Email atau NRP</label>
          <input
            id="nrp"
            v-model="form.nrp"
            type="text"
